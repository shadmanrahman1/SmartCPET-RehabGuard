...
def _aggregate(rows: list[dict]) -> dict:
    ...
    Availability is reported as BOTH a macro-mean of per-video rates and an
    overall (pooled) rate when raw counts are present. The p95 is reported as
    the MEAN OF per-video p95 values — it is never labelled a global p95 (a
    true global p95 would require pooled per-frame latency samples, which are
    not available here).
    """
    ...
    video_p95 = [float(r["p95_ms_per_frame"]) for r in ok if r.get("p95_ms_per_frame") is not None]
    throughputs = [r["effective_throughput_fps"] for r in ok if r.get("effective_throughput_fps") is not None]

    # Weighted (pooled) availability when raw counts are available.
    total_valid = sum(int(r.get("valid_pose_frames") or 0) for r in ok)
    total_world = sum(int(r.get("world_landmark_available_frames") or 0) for r in ok)
    overall_availability = round(total_world / total_valid, 4) if total_valid else None

    macro_rates = [
        float(r["world_landmark_availability_rate"])
        for r in ok
        if r.get("world_landmark_availability_rate") is not None
    ]
    ...
    return {
        ...
        "n_success": len(ok),
        "aggregate": {
            "total_frames": sum(int(r["total_frames"]) for r in ok),
            "macro_mean_world_landmark_availability_rate": (
                round(sum(macro_rates) / len(macro_rates), 4) if macro_rates else None
            ),
            "overall_world_landmark_availability_rate": overall_availability,
            ...
            "mean_of_video_p95_ms_per_frame": (
                round(statistics.fmean(video_p95), 4) if video_p95 else None
            ),
...
def run_benchmark_batch(input_dir: Optional[Path], output_dir: Path, fps_override=None) -> dict:
    ...
    for video in videos:
        ...
        try:
            ...
            # NOTE: no filename is persisted (a filename may carry a participant
            # identifier). Only the opaque sequence_key is used.
    ...
    with open(output_dir / "benchmark_batch.csv", "w", newline="", encoding="utf-8") as fh:
        writer = csv.DictWriter(
            fh,
            fieldnames=[
                "sequence_key", "status", "data_origin", "fps_used_hz",
